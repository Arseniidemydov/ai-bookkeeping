...
export function Dashboard() {
  ...
  useEffect(() => {
    const channel = supabase.channel('table-db-changes').on('postgres_changes', {
      event: '*',
      schema: 'public',
      table: 'transactions'
    }, () => {
  ...
  return <div className={cn("fixed top-0 left-0 right-0 z-50 bg-gray-900/80 backdrop-blur-xl border-b border-white/10 transition-all duration-300 ease-in-out", isExpanded ? "h-screen" : "h-64" // Increased from h-48 to h-64
  )}>
      <div className="p-4 h-full overflow-y-auto py-[24px]">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {visibleMetrics?.map(metric => <div key={metric.label} className={cn("p-4 rounded-2xl backdrop-blur-sm border transition-all duration-300 h-auto",
        // Added h-auto
        metric.type === 'income' && "bg-emerald-950/30 border-emerald-800/50", metric.type === 'expense' && "bg-rose-950/30 border-rose-800/50", metric.type === 'tax' && "bg-amber-950/30 border-amber-800/50", metric.type === 'net' && "bg-blue-950/30 border-blue-800/50")}>
