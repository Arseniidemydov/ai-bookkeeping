...
export function ConversationStarters({ onSelect }: ConversationStartersProps) {
  return (
    <div className="sticky top-48 left-0 right-0 px-4 py-2 z-10">
      <div className="max-w-2xl mx-auto flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {starters.map((starter) => (
          <button
            key={starter.text}
            onClick={() => onSelect(starter.text)}
            className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-white/5 hover:bg-white/10 text-xs text-white/80 whitespace-nowrap transition-colors border border-white/10"
          >
            {starter.icon}
